...
class BandwidthStorage:
    ...
    def get_top_processes(self, hours: int = 1, limit: int = 10) -> List[Dict]:
        """Get top processes by bandwidth usage in the last N hours"""
    ...
    def get_protocol_breakdown(self, hours: int = 1) -> Dict:
        ...
        for row in cursor.fetchall():
            results[row['protocol']] = {
                'tx_bytes': row['total_tx'],
                'rx_bytes': row['total_rx']
            }
    ...
    def get_ip_breakdown(self, process_name: str = None, hours: int = 1) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            results.append({
                ...
                'process_name': row.get('process_name', 'N/A'),
                'total_tx': row['total_tx'],
                'total_rx': row['total_rx']
            })
            ...
            if bucket_time != current_bucket:
                ...
                if bucket_data['count'] > 0:
                    results.append({
                        ...
                        'tx_bytes': bucket_data['tx'],
                        'rx_bytes': bucket_data['rx']
                    })
